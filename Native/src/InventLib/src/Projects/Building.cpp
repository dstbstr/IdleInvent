...
namespace {
    ...
    std::vector<Effect> MakeEffects(ResourceName specialty, size_t level) { 
        ...
        if(level == 6) {
            ...
            case ResourceName::Followers:
                result.push_back(Effect{EffectTarget::FollowerCap, Effects::Increase::Mul::Small});
                break;
            case ResourceName::Knowledge:
				result.push_back(Effect{EffectTarget::KnowledgeCap, Effects::Increase::Mul::Small});
				break;
            case ResourceName::Money:
                result.push_back(Effect{EffectTarget::MoneyCap, Effects::Increase::Mul::Small});
                break;
            case ResourceName::Power:
                result.push_back(Effect{EffectTarget::PowerCap, Effects::Increase::Mul::Small});
				break;
        ...
        if (level == 7) {
            switch(specialty) {
            case ResourceName::Followers:
                result.push_back(Effect{EffectTarget::FollowerRate, Effects::Increase::Mul::Small});
                break;
            case ResourceName::Knowledge:
                result.push_back(Effect{EffectTarget::KnowledgeRate, Effects::Increase::Mul::Small});
                break;
            case ResourceName::Money:
                result.push_back(Effect{EffectTarget::MoneyRate, Effects::Increase::Mul::Small});
                break;
            case ResourceName::Power:
                result.push_back(Effect{EffectTarget::PowerRate, Effects::Increase::Mul::Small});
                break;
            default: DR_ASSERT_MSG(false, "Unhandled specialty"); break;
            }
        }
    ...
    ResourceCollection MakeResource(size_t followers, size_t knowledge, size_t money, size_t power) {
        ResourceCollection result{};
        result.at(ResourceName::Followers).Current = Costs[followers];
        result.at(ResourceName::Knowledge).Current = Costs[knowledge];
        result.at(ResourceName::Money).Current = Costs[money];
        result.at(ResourceName::Power).Current = Costs[power];
        result.at(ResourceName::Followers).Capacity = Costs[followers];
        result.at(ResourceName::Knowledge).Capacity = Costs[knowledge];
        result.at(ResourceName::Money).Capacity = Costs[money];
        result.at(ResourceName::Power).Capacity = Costs[power];

        return result;
    }

    // 10 17 24 32 39 46 54 61 68 75 82 90 97 104 111 118 126 133 140 147 156
    std::unordered_map<Building, ResourceCollection> BuildingBaseResourceCost{
